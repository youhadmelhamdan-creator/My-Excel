...
If IP_ADD = "" Or (Plan <> "ADMIN" And Plan <> "GOLD" And Plan <> "VIP" And Plan <> "SILVER" And Plan <> "BRONZE") Then ' Checking The IP & Plan Validity '
    wb2.Close                       ' FIX: was Workbooks("Version").Close — the open book is "Version.xlsm", so the name without extension raised error 9 and (because "On Error GoTo Internet" is still active) showed a misleading "network problem" message instead of the authorization warning. wb2 is the same workbook object.
